...
public class BackUpGUI extends JFrame{
	...
	private void leerContactos() {
		...
		String ag;
		String accion="select * from contactos;";
		...
		try{
			...
			ruta.mkdirs();
			setDf(new SimpleDateFormat("ddMMyyyy HHmmss"));
			GregorianCalendar currCal = (GregorianCalendar) Calendar.getInstance();

			File archivo=new File(ruta, "backCt "+df.format(currCal.getTime()) +".csv");
			...
			while (rs.next()){
				...
				ct.setTelefonos(rs.getString(4));
				ct.setEmail(rs.getString(5));
				ct.setComentario(rs.getString(6));
				ag=ct.getNombre()+"; "+ct.getDireccion()+"; "+ct.getTelefonos()+"; "+ct.getEmail()+"; "+ct.getComentario();
	...
	private void leerAgentes() {
		...
		try{
			...
			File archivo=new File(ruta, "backAg "+df.format(currCal.getTime()) +".csv");
			...
			while (rs.next()){
				...
				ag=agente.getLegajo()+"; "+agente.getNombreyApellido()+"; "+agente.getFechaNac()+"; "+agente.getTipoDni()+"; "+agente.getDni()+"; "+
				agente.getDireccion()+"; "+agente.getTelefono()+"; "+agente.getEmail()+"; "+agente.getNombreGrupo();
